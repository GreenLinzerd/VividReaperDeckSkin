local atlas_key = 'vividreaper_atlas' -- "{prefix}_atlas"
...
local description = 'vividReaper Deck' -- English-language description, also used as default. Can also be a table for each suit.

-----------------------------------------------------------
-- You should only need to change things above this line --
-----------------------------------------------------------

-- Atlases for each suit
for i, suit in ipairs(suits) do
    SMODS.Atlas {
        key = atlas_key .. '_' .. string.lower(suit) .. '_lc',
        px = 71,
        py = 95,
        path = atlas_lc_img[i]
    }
    ...
    if atlas_hc_img then
        SMODS.Atlas {
            key = atlas_key .. '_' .. string.lower(suit) .. '_hc',
            px = 71,
            py = 95,
            path = atlas_hc_img[i]
        }
...
for i, suit in ipairs(suits) do
    ...
    local lc_palette = {
        ...
        atlas = atlas_key .. '_' .. string.lower(suit) .. '_lc',
        pos_style = "suit"
    }
    ...
    if atlas_hc_img then
        local hc_palette = {
            ...
            ranks = ranks_to_use,
            display_ranks = ranks_to_use,
            atlas = atlas_key .. '_' .. string.lower(suit) .. '_hc',
            pos_style = "suit",
            hc_default = true,
        }
